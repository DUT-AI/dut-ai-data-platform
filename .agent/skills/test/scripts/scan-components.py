...
def scan_commands() -> list[dict[str, Any]]:
    """Scan all commands in .agent/commands/"""
    commands = []
    cmd_path = BASE_PATH / ".agent" / "commands"
...
def scan_agents() -> list[dict[str, Any]]:
    """Scan all agents in .agent/agents/"""
    agents = []
    agent_path = BASE_PATH / ".agent" / "agents"
...
def scan_skills() -> list[dict[str, Any]]:
    """Scan all skills in .agent/skills/"""
    skills = []
    skill_path = BASE_PATH / ".agent" / "skills"
...
def scan_workflows() -> list[dict[str, Any]]:
    """Scan all workflows in .agent/workflows/"""
    workflows = []
    wf_path = BASE_PATH / ".agent" / "workflows"
...
def main():
    ...
    parser.add_argument(
        "--scenarios", action="store_true", help="Generate test scenarios"
    )
    parser.add_argument(
        "--type",
        choices=["commands", "agents", "skills", "workflows", "all"],
        default="all",
        help="Component type to scan",
    )
    ...
    if args.scenarios:
        result["scenarios"] = generate_test_scenarios(
            result.get("commands", []), result.get("skills", [])
        )
