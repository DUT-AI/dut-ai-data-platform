            ...
            with open(env_path) as f:
                for line in f:
                    ...
                    if (
                        ...
                        and line.startswith("GEMINI_API_KEY=")
                    ):
...
def build_prompt(
    ...
    if brand or brand_colors:
        ...
        if brand:
            parts.append(
                f'- Channel/brand name: "{brand}" — include small logo/watermark in top-left corner.'
            )
        if brand_colors:
            parts.append(
                f"- Brand color palette: {brand_colors}. Use these as primary colors throughout the design."
            )
        sections.append("\n".join(parts))

    if ref_path:
        sections.append(
            "FACE REFERENCE: Use the provided reference face photo as the person in this thumbnail. Keep their likeness accurate. Match their appearance exactly."
        )

    if arrows:
        sections.append(
            "ARROWS: Include bold graphic arrows (red or bright yellow) pointing at the key subject or text to draw the viewer's eye. Arrows should be hand-drawn style or thick graphic arrows, highly visible."
        )
...
def generate_thumbnail(
    ...
    if not GEMINI_API_KEY:
        print(
            "Error: GEMINI_API_KEY not set. Set with: export GEMINI_API_KEY='your-key'"
        )
        return None
    ...
    print(
        f'Style: {style or "bold-text"} | Niche: {niche or "general"} | Text: "{text}" | Font: {font or DEFAULT_FONT}'
    )
    if brand:
        print(
            f"Brand: {brand}" + (f" | Colors: {brand_colors}" if brand_colors else "")
        )
    print()
    ...
    contents.append(
        build_prompt(
            topic, text, style, niche, font, brand, brand_colors, arrows, ref_path
        )
    )
    ...
    try:
        ...
        if output_path is None:
            ...
            output_path = (
                f"thumb_{style_slug}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
...
def generate_batch(
    ...
    print(
        f"  Niche: {niche or 'general'} | Arrows: {'yes' if arrows else 'no'} | Variants: {count}"
    )
...
def main():
    ...
    parser.add_argument(
        "--prompt", "-p", type=str, help="Video topic / thumbnail description"
    )
    parser.add_argument(
        "--text", "-t", type=str, help="Bold text to render (1-3 words, max 25 chars)"
    )
    parser.add_argument(
        "--style", "-s", choices=list(STYLE_PROMPTS.keys()), help="Thumbnail style"
    )
    parser.add_argument(
        "--niche", "-n", choices=list(NICHE_AESTHETICS.keys()), help="Content niche"
    )
    parser.add_argument(
        "--font",
        "-f",
    ...
    parser.add_argument(
        "--brand", "-b", type=str, help="Brand/channel name for watermark"
    )
    parser.add_argument(
        "--brand-colors", type=str, help="Brand colors as comma-separated hex codes"
    )
    parser.add_argument(
        "--arrows", action="store_true", help="Add bold arrows pointing at key subject"
    )
    parser.add_argument("--ref", type=str, help="Reference face image path")
