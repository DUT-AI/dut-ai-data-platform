...
def format_output(result):
    ...
    output = []
    output.append("## YouTube Thumbnail Search Results")
...
def generate_design_brief(query, video_title=None):
    ...
    if "style" in results:
        ...
        for r in results["style"]:
            output.append(
                f"**{r.get('Style Name', 'N/A')}** ({r.get('Category', 'N/A')})"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Search YouTube thumbnail design guidelines"
    )
    parser.add_argument("query", type=str, help="Search query")
    parser.add_argument(
        "--domain", "-d", choices=list(CSV_CONFIG.keys()), help="Domain to search"
    )
    parser.add_argument(
        "--max-results", "-n", type=int, default=MAX_RESULTS, help="Max results"
    )
    parser.add_argument(
        "--design-brief", action="store_true", help="Generate full design brief"
    )
    parser.add_argument(
        "--video-title", "-t", type=str, help="Video title for design brief"
    )
