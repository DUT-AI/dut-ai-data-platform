...
@patch("apps.api.main.check_rabbitmq", new_callable=AsyncMock)
@patch("apps.api.main.check_minio", new_callable=AsyncMock)
def test_readiness_check_success(mock_minio, mock_rabbitmq, mock_redis, mock_db):
    mock_db.return_value = (True, "ok")
    mock_redis.return_value = (True, "ok")
    mock_rabbitmq.return_value = (True, "ok")
...
@patch("apps.api.main.check_rabbitmq", new_callable=AsyncMock)
@patch("apps.api.main.check_minio", new_callable=AsyncMock)
def test_readiness_check_unhealthy(mock_minio, mock_rabbitmq, mock_redis, mock_db):
    mock_db.return_value = (True, "ok")
