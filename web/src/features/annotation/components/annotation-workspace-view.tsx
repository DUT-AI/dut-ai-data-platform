...
import { useAssetDownloadUrlQuery, useVersionAssetsQuery } from "@/features/dataset/hooks/use-datasets";
...
export function AnnotationWorkspaceView({
  ...
  // Queries
  const { data: annotations, isLoading: isAnnoLoading } = useAssetAnnotationsQuery(assetId);
  const { data: downloadData, isLoading: isDownloadLoading } = useAssetDownloadUrlQuery(assetId);
  const { data: assets, isLoading: isAssetsLoading } = useVersionAssetsQuery(datasetVersionId || "");

  const downloadUrl = downloadData?.download_url;
  const activeAnnotation = annotations?.[0];
  const revisions = useMemo(() => activeAnnotation?.revisions || [], [activeAnnotation]);
  ...
  const isLoading = isAnnoLoading || isDownloadLoading || isAssetsLoading || isFetchingUrl;
    ...
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-slate-950 text-slate-100">
      ...
      <header className="flex h-14 items-center justify-between border-b border-slate-800 bg-slate-900 px-6">
        ...
        <div className="flex items-center space-x-4">
          <Link
            ...
            className="flex items-center space-x-1.5 text-xs font-medium text-slate-400 hover:text-slate-200 transition-colors"
          >
            <span>←</span>
            <span>Quay lại Dataset</span>
          </Link>
          <div className="h-4 w-px bg-slate-800" />
          <span className="font-mono text-sm font-semibold truncate max-w-[200px]" title={assetFilename}>
            📄 {assetFilename}
          </span>
        </div>
        ...
        {assets && assets.length > 0 && currentAssetIdx !== -1 && (
          <div className="flex items-center space-x-3">
            <Button
              ...
              className="h-8 border-slate-800 px-3 text-xs bg-slate-950 text-slate-300 hover:bg-slate-900"
            >
              ◀ Trước
            </Button>
        ...
        <div className="flex items-center space-x-3">
          <Button
            ...
            disabled={!taskUrl}
            className="h-8 border-slate-800 text-xs bg-slate-950 text-slate-300 hover:bg-slate-900"
          >
            ↗ Mở Tab Mới (Full tab)
          </Button>
          <Button
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="relative flex flex-1 flex-col bg-slate-950">
          {lsError && (
            <div className="absolute left-6 right-6 top-6 rounded-lg bg-red-950/40 p-4 border border-red-900/50 text-sm text-red-400 z-50">
              ⚠️ {lsError}
            </div>
          )}

          {isLoading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-3 bg-slate-950/90 z-40">
            ...
            <div className="flex h-full w-full items-center justify-center text-sm text-slate-500 font-mono">
        ...
        {isSidebarOpen && (
          <aside className="flex w-80 flex-col border-l border-slate-800 bg-slate-900 p-4 overflow-y-auto space-y-4">
